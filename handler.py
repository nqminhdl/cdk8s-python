...
class Yamldata():
    ...
    def return_generate_manifest(self, service_name, dist_file_path):
        ...
        for i in generated_data:
            if service_name in i['metadata']['name']:
                Path(
                    f"./dist/{service_name}").mkdir(parents=True, exist_ok=True)
                kind = i['kind'].lower()
                file_name = f'./dist/{service_name}/{kind}.yaml'
        ...
        kustomization_template['resources'] = listdir(
            path=f'./dist/{service_name}')
        with open(f'./dist/{service_name}/kustomization.yaml', 'w') as file:
